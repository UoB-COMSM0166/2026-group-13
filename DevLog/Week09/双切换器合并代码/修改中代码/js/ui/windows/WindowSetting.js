...
export class WindowSetting extends WindowBase {
  ...
  _buildContent() {
    ...
    try {
      if (this.layout === 'tabbed') {
        this._buildTabbedContent(area);
      } else {
        this._buildStackedContent(area);
      }
    } catch (err) {
  ...
  _buildStackedContent(area) {
    this._buildSoundContent(area);
    this._buildLanguageContent(area);
    this._buildKeybindContent(area);
  }

  _buildTabbedContent(area) {
    const p = this.p;

    this._layoutRoot = p.createDiv('');
    this._layoutRoot.addClass('window-setting-layout');
    this._layoutRoot.parent(area);

    this._navArea = p.createDiv('');
    this._navArea.addClass('window-setting-nav');
    this._navArea.parent(this._layoutRoot);

    this._panelArea = p.createDiv('');
    this._panelArea.addClass('window-setting-panels');
    this._panelArea.parent(this._layoutRoot);

    const soundPanel = this._createSectionPanel('sound');
    this._buildSoundContent(soundPanel);

    const languagePanel = this._createSectionPanel('language');
    this._buildLanguageContent(languagePanel);

    if (this.keyBindingManager) {
      const keybindPanel = this._createSectionPanel('keybind');
      this._buildKeybindContent(keybindPanel);
    }
  ...
  _createSectionPanel(sectionKey) {
    ...
    panel.addClass('window-setting-panel');
    panel.parent(this._panelArea);
    this._sectionPanels[sectionKey] = panel;
    return panel;
  }

  _createSectionButton(sectionKey, label) {
    const button = this.p.createButton(label);
    button.addClass('window-setting-tab');
    button.parent(this._navArea);
    button.mousePressed(() => this._setActiveSection(sectionKey));
    this._sectionButtons[sectionKey] = button;
  }

  _setActiveSection(sectionKey) {
    this._activeSection = sectionKey;

    for (const [key, button] of Object.entries(this._sectionButtons)) {
      if (key === sectionKey) {
  ...
  _buildKeybindContent(parent) {
    ...
    for (const [intent, keyCode] of Object.entries(config)) {
  ...
  _onKeyBindChange(intent, newKeyCode) {
    if (intent === null) {
      // 完全重置，更新所有按钮
      const config = this.keyBindingManager.getConfig();
      for (const [i, keyCode] of Object.entries(config)) {
